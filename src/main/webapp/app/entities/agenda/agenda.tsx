...
import { getEntities, reset } from './agenda.reducer';
import { IAgenda } from 'app/shared/model/agenda.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';
import { ITEMS_PER_PAGE } from 'app/shared/util/pagination.constants';

export interface IAgendaProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export type IAgendaState = IPaginationBaseState;
...
export class Agenda extends React.Component<IAgendaProps, IAgendaState> {
  ...
  render() {
    const { agendaList, match } = this.props;
    return (
      <div>
              ...
              <Table responsive>
                ...
                <tbody>
                  {agendaList.map((agenda, i) => (
                    <tr key={`entity-${i}`}>
                      <td>
                        <Button tag={Link} to={`${match.url}/${agenda.id}`} color="link" size="sm">
                          {agenda.id}
                        </Button>
                      </td>
                      <td>{agenda.attendance ? 'true' : 'false'}</td>
                      <td>{agenda.activityScoring}</td>
                      <td>{agenda.moderatorScoring}</td>
                      <td>{agenda.userProfile ? <Link to={`user-profile/${agenda.userProfile.id}`}>{agenda.userProfile.id}</Link> : ''}</td>
                      <td className="text-right">
                        <div className="btn-group flex-btn-group-container">
                          <Button tag={Link} to={`${match.url}/${agenda.id}`} color="info" size="sm">
                            <FontAwesomeIcon icon="eye" />{' '}
                            <span className="d-none d-md-inline">
                              <Translate contentKey="entity.action.view">View</Translate>
                            </span>
                          </Button>
                          <Button tag={Link} to={`${match.url}/${agenda.id}/edit`} color="primary" size="sm">
                            <FontAwesomeIcon icon="pencil-alt" />{' '}
                            <span className="d-none d-md-inline">
                              <Translate contentKey="entity.action.edit">Edit</Translate>
                            </span>
                          </Button>
                          <Button tag={Link} to={`${match.url}/${agenda.id}/delete`} color="danger" size="sm">
...
const mapStateToProps = ({ agenda }: IRootState) => ({
  agendaList: agenda.entities,
  totalItems: agenda.totalItems,
  links: agenda.links,
  entity: agenda.entity,
  updateSuccess: agenda.updateSuccess
});
