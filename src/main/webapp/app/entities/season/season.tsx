...
import { getEntities, reset } from './season.reducer';
import { ISeason } from 'app/shared/model/season.model';
// tslint:disable-next-line:no-unused-variable
import { APP_DATE_FORMAT, APP_LOCAL_DATE_FORMAT } from 'app/config/constants';
import { ITEMS_PER_PAGE } from 'app/shared/util/pagination.constants';

export interface ISeasonProps extends StateProps, DispatchProps, RouteComponentProps<{ url: string }> {}

export type ISeasonState = IPaginationBaseState;
...
export class Season extends React.Component<ISeasonProps, ISeasonState> {
  ...
  render() {
    const { seasonList, match } = this.props;
    return (
      <div>
              ...
              <Table responsive>
                ...
                <tbody>
                  {seasonList.map((season, i) => (
                    <tr key={`entity-${i}`}>
                      <td>
                        <Button tag={Link} to={`${match.url}/${season.id}`} color="link" size="sm">
                          {season.id}
                        </Button>
                      </td>
                      <td>{season.name}</td>
                      <td>
                        <TextFormat type="date" value={season.startDate} format={APP_DATE_FORMAT} />
                      </td>
                      <td>
                        <TextFormat type="date" value={season.endDate} format={APP_DATE_FORMAT} />
                      </td>
                      <td className="text-right">
                        <div className="btn-group flex-btn-group-container">
                          <Button tag={Link} to={`${match.url}/${season.id}`} color="info" size="sm">
                            <FontAwesomeIcon icon="eye" />{' '}
                            <span className="d-none d-md-inline">
                              <Translate contentKey="entity.action.view">View</Translate>
                            </span>
                          </Button>
                          <Button tag={Link} to={`${match.url}/${season.id}/edit`} color="primary" size="sm">
                            <FontAwesomeIcon icon="pencil-alt" />{' '}
                            <span className="d-none d-md-inline">
                              <Translate contentKey="entity.action.edit">Edit</Translate>
                            </span>
                          </Button>
                          <Button tag={Link} to={`${match.url}/${season.id}/delete`} color="danger" size="sm">
...
const mapStateToProps = ({ season }: IRootState) => ({
  seasonList: season.entities,
  totalItems: season.totalItems,
  links: season.links,
  entity: season.entity,
  updateSuccess: season.updateSuccess
});
